...
# Needs update!!
makeData_forMap <- function(regime_name, ...) {
...
d1 <- lapply(my_regimes, makeData_forMap)
...
p.ext<- 
  ...
  geom_point(data = df_join_ext, 
             aes(x = X_10,
                 y = Padded_id, 
                 color = perc_change_risk  ), 
             size = 1,
             shape = )  +  # risk_cat
  scale_colour_gradient2(limits=c(-100, 100),
                         low = "#a50026", 
                         mid = NA, 
                         high = "#313695", 
                         midpoint = 0, 
                         breaks = breaks) +
  ggtitle("Extended BAU (30%)") +
  ylab('') +
  xlab('') +
  theme_map()
...
ggarrange(p.short, p.ext, ncol = 2, common.legend = TRUE)
...
ggplot(df_join) + 
  geom_point(#data = df.geom, 
    aes(x = X_10,
        y = Padded_id, 
        color = perc_change_HSI)) #+
