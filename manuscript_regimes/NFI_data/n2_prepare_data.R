...
names = c("rslt_FBE_rcp0_f") #
names = c("rslt_FBE_rcp45_f", "rslt_FBE_rcp85_f" )
...
addXYVals <- function(df_name, ...) {
  #gc()

    # Read over loop
  #df_name = "rslt_FBE_rcp0_f"
  in_name = paste0(df_name, '.csv')
  out_name = paste0(gsub(pattern = "rslt_FBE_", 
                         replacement = "", 
                         x = df_name))
...
# Create and empty list to store the outputs
my_ls <- ls()

my_ls <- lapply(names, addXYVals) 
...
# Merge simulatd data with raster values 
df <- data.table::fread(paste(inPath, in_folder, in_name, sep = "/"),  # 
                        data.table=TRUE)
