...
![The study are located in Central Finland (watershed 14.534) comprising 1475 forest stands.](/MyTemp/myGitLab/windDamage/externalFigs/studyArea_crop.png)
...
``` {r fig3_mean_risk_by_intensity_plot, fig.width = 4, fig.height = 3.5, fig.cap = "Mean wind risk damage for three types of management regimes over  harvest intensity gradient"}
...
```{r fig_6_count_open_edge, eval = T, echo = F, warning = F, message = F, fig.width = 4, fig.height = 2, fig.cap = "Yearly count of stands with open edge over the intensity gradient"}
...
df %>% 
 ...
 # summarize(mean.m = mean(n)) #%>% 
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        legend.position = "bottom") +
  ggtitle("") +
  labs(color = "Regimes group") 

```

### Changes in species composition

```{r fig_7_species_change, eval = T, echo = F, warning = F, message = F, fig.width = 7, fig.height = 3, fig.cap = "Changes in species composition under different management types and harvest intensity"}
...
#p.sp.2016 <- 
  df %>%
  filter(year == 2016) %>%
  group_by(simpleScen, scenNumb, species) %>% 
