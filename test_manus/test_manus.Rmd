...
Future forest ecosystems need to simultaneously provide timber, non-woody ecosystem services such as carbon storage or human recreation, and shelter biodiversity.  Adaptive forest management strives to balance between timber production, provision of non-woody ecosystem services, and biodiversity. Existance of biodiversity, mostly attached to existence of deadwood, is limited by harvesting levels. Intensive logging activities fragment forested landspaces. 
...
```{r proportion_V_pulp_log, eval = T, echo = F, warning = F, message = F, fig.width = 4, fig.height = 2, fig.cap = "Ratio between standing log and pul volume (m^3)"}
...
### Temporal changes in wind risk over the landscape

```{r fig_windRisk_time_change, fig.width = 4, fig.height = 3, fig.cap = "Temporal changes in wind risk over time by management groups and harvest intensity."}
...
```{r scatter_risk_by_volume_year, eval = F} 

# who does the wind risk changes by volume among the regimes??
...
# Dummy example to chcek how does identifucation iof teh non NA values work
...
Our study design have multiple limitation, inhereted from application of teh pixel (30x30m) resolution to stand level (mean: 1.5 ha). This likely overestimated the volume under risk due to larger computational unit. However, Finnish forests stands are relatively homogenous in terms of species composition, tree height and age composition. Further, scarce windthrown damage or tree breakage due to wind or snow occurs annualy, while rare extreme winds could decimate all mature trees available on stands. As such, the expectation of the rate of wind damage vary.   



The shortening of the rotation period in Norway spruce forets  might slightly lower the wind and subsetquent bark beetle distudrabnces but has limited efficiencies and decreses under climate change [@Zimova2020]
...
recalculate teh datasets from the highest stratum, and supposed that this stratum will be the most affected if damaged?? Proportion between the V of tallest stratum and and total volume of the stand could shed light on the how much timber volume is at risk at certain time.
